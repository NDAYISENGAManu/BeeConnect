...
import { employmentSource } from "../reusable/data";
import { api } from "../api";
import {
  getEducationLevel,
  getBusinessType,
...
const EmploymentReport: React.FC = () => {
  ...
  const getEmploymentsReport = async () => {
    setLoading(true);
    try {
      const { data } = await api.get(
        "/api/v1/applicants/employment-reports?outputFormat=json",
        {
          params: {
            ...filters,
            page: currentPage,
      ...
      if (data.status == 200) {
        const employment = data.data.data;
        employment.map((el: any) => {
  ...
  // outputFormat: Joi.string().valid("json", "file").default("json").optional(),

  const onDownloadClick = async () => {
    try {
      const { data } = await api.get(
        `/api/v1/applicants/employment-reports?outputFormat=file`,
        {
          params: {
            ...filters,
            page: currentPage,
